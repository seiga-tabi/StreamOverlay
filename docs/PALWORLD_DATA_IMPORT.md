...
## 운영자 PAK Content + delta candidate workflow

운영자 FModel export는 기존 release와 섞어 활성화하지 않고 다음 두 archive를 고정 입력으로 사용하는 candidate로 먼저 생성한다.

| 역할 | 입력 | SHA-256 |
| --- | --- | --- |
| primary 데이터·한국어 locale | `Content.zip` | `1248184a4b527d947b5411940726d5b41fa0e212b355b7e4cc917821e0496384` |
| 고정 asset overlay | `delta.zip` | `2108e7bd6029117473b2ff38d5c9884ec6b717af7645b11e4db44b9b0459a443` |

overlay는 고정 SHA-256을 제공한 경우에만 대용량 profile로 연다. 같은 member가 두 archive에 있으면 실제 bytes가 완전히 동일할 때만 허용하고, 크기나 내용이 다르면 import를 실패시킨다. 따라서 `delta.zip`은 primary의 DataTable이나 locale을 덮어쓸 수 없으며 신규 member만 보충한다.

```bash
npm run import:palworld-pak --workspace @streamops/server -- \
  --archive <Content.zip> \
  --expected-sha256 1248184a4b527d947b5411940726d5b41fa0e212b355b7e4cc917821e0496384 \
  --asset-overlay <delta.zip> \
  --asset-overlay-sha256 2108e7bd6029117473b2ff38d5c9884ec6b717af7645b11e4db44b9b0459a443 \
  --mappings-usmap-member Mappings101.usmap \
  --official-en optional \
  --metadata <palworld-export-metadata.json> \
  --legacy-catalog <검증된-catalog.json> \
  --output <candidate-directory>
```

`--official-en optional`은 KO/JA 공개 release에서 공식 EN locale을 activation 필수 조건에서만 제외한다. 영어 값을 한국어·일본어 필드로 복사하거나 기존 기계 번역을 공식 번역으로 승격하지 않는다.

`delta.zip`의 `Mappings101.usmap`은 `2,342,284 bytes`, SHA-256 `b9b1ef0a5b2fba657d421f43c31dda7d3818aadb50e52c1c3215dfe45db9fb44`이다. 별도 metadata 파일은 다음 exact schema를 사용하며 실제 값은 파일명이나 레코드 수로 추정하지 않는다.

```json
{
  "schemaVersion": 1,
  "sourceType": "operator_pak_export",
  "gameVersion": "<검증된-semver>",
  "steamBuildId": "<검증된-Steam-Build-ID>",
  "fmodelVersion": "<검증된-FModel-version>",
  "exportedAt": "<고정-RFC3339>",
  "mappingsSha256": "b9b1ef0a5b2fba657d421f43c31dda7d3818aadb50e52c1c3215dfe45db9fb44"
}
```

각 `<...>` 값은 예시 placeholder이므로 실제 metadata에는 사용할 수 없다. 현재 overlay에는 `DT_ItemIconDataTable`이 가리키는 `Others/InventoryItemIcon/Texture/`가 없어 item image gate를 통과하지 못한다. 또한 `PickAxe_Default`를 `Pickaxe_Default`로 연결해야 하는 세 legal item row는 fuzzy/case 추측 대신 release와 source checksum에 묶인 versioned exact alias가 필요하다.
