...
- 신규 `/login`은 Discord 또는 Twitch identity로 동일한 YORO session을 발급하며, `/bot/manage`는 이 session을 우선 사용합니다.
