...
현재 Discord 기반 migration은 onboarding `0004`, Bot binding `0005`, Organization 관리 session과 Agent bootstrap `0006`, Agent credential·nonce·status idempotency 기반 `0007`, 웹 management Guild claim 목적 기반 `0008`입니다. 기존에 적용한 `0001`~`0007` SQL은 수정하지 않았습니다. staging에서 `check`와 `plan`을 확인한 뒤 별도 승인으로 pending migration을 적용합니다.
