...
const botText = {
  ko: {
    ...
    featureStatusDescription: "Palworld 서버 등록과 Agent 설치 준비까지만 제공하며, 실제 Agent 상태 수집은 다음 단계에서 제공합니다.",
    featureNotification: "상태 알림",
    featureNotificationDescription: "중복 방지와 tenant 격리를 적용한 알림 Worker를 후속 단계에서 연결합니다.",
    available: "기반 완료",
    planned: "준비 중",
    flowTitle: "연결 과정",
    flowDescription: "설정 링크를 받은 뒤 다음 순서로 Discord 서버를 Organization에 연결합니다.",
    flowIssue: "설정 링크 발급",
    flowIssueDescription: "YORO Bot의 `/yoro setup` 명령에서 Guild와 실행자에게 귀속된 10분 일회용 링크를 발급합니다.",
    ...
    setupNotice: "Bot Gateway와 `/yoro setup` 구현은 완료됐지만 운영 command 등록과 feature flag 활성화 전에는 사용할 수 없습니다. 설정 링크는 Discord의 ephemeral 명령 응답으로만 제공합니다.",
    ...
    nextTitle: "다음 구현 단계",
    nextDescription: "Agent 실행 파일과 상태 Ingestion API, 알림 Worker 순서로 확장합니다.",
    privacy: "개인정보 처리방침",
    terms: "이용약관",
    contact: "문의",
    disclaimer: "Organization 관리와 Agent 설치 토큰 기반은 준비됐지만, 실제 Agent 실행과 게임 서버 상태 수집은 아직 제공되지 않습니다.",
  ...
  ja: {
    ...
    featureStatusDescription: "Palworldサーバー登録とAgent導入準備まで提供し、実際のAgent状態収集は次の段階で提供します。",
    featureNotification: "状態通知",
    featureNotificationDescription: "重複防止とtenant分離を適用した通知Workerを後続段階で連携します。",
    available: "基盤完了",
    planned: "準備中",
    flowTitle: "連携手順",
    flowDescription: "設定リンクを受け取った後、次の順序でDiscordサーバーをOrganizationに連携します。",
    flowIssue: "設定リンク発行",
    flowIssueDescription: "YORO Botの `/yoro setup` コマンドからGuildと実行者に紐づく10分間有効なワンタイムリンクを発行します。",
    ...
    setupNotice: "Bot Gatewayと `/yoro setup` の実装は完了していますが、運用command登録とfeature flag有効化前は利用できません。設定リンクはDiscordのephemeral応答でのみ提供します。",
    ...
    nextTitle: "次の実装段階",
    nextDescription: "Agent実行ファイル、状態Ingestion API、通知Workerの順に拡張します。",
    privacy: "プライバシーポリシー",
    terms: "利用規約",
    contact: "お問い合わせ",
    disclaimer: "Organization管理とAgent導入トークン基盤は準備済みですが、実際のAgent実行とゲームサーバー状態収集はまだ提供されていません。",
    copyright: "Copyright © 2026 YORO.gg",
  },
} as const;
        ...
        <section className="public-bot-hero" id="bot-overview">
          <div className="public-bot-hero__copy">
            ...
            <div className="public-bot-actions">
              <a className="public-bot-button is-primary" href="#bot-features">{text.explore}</a>
              <a className="public-bot-button" href="#bot-flow">{text.setupGuide}</a>
            </div>
          </div>
