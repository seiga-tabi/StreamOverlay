...
import type { PalworldItemDetail, PalworldPalDetail, PalworldSkill } from "@streamops/shared";
...
type DescriptionReference = { descriptionKo?: string; descriptionJa?: string; descriptionEn?: string };
...
function referenceName(value: NameReference, locale: PalworldLocale): string {
  return (locale === "ja" ? value.nameJa : value.nameKo)?.trim() || value.nameEn;
}

function usesEnglishNameFallback(value: NameReference, locale: PalworldLocale): boolean {
  return !((locale === "ja" ? value.nameJa : value.nameKo)?.trim()) && Boolean(value.nameEn.trim());
}
...
function acquisitionText(method: { labelKo?: string; labelJa?: string; labelEn?: string }, locale: PalworldLocale): string {
  return (locale === "ja" ? method.labelJa : method.labelKo)?.trim() || method.labelEn?.trim() || palworldI18n[locale].originalDataUnavailable;
}

function acquisitionLocation(method: { locationKo?: string; locationJa?: string; locationEn?: string }, locale: PalworldLocale): string | undefined {
  return (locale === "ja" ? method.locationJa : method.locationKo)?.trim() || method.locationEn?.trim() || undefined;
}

function usesEnglishAcquisitionFallback(
  method: { labelKo?: string; labelJa?: string; labelEn?: string; locationKo?: string; locationJa?: string; locationEn?: string },
  locale: PalworldLocale,
): boolean {
  const localizedLabel = (locale === "ja" ? method.labelJa : method.labelKo)?.trim();
  const localizedLocation = (locale === "ja" ? method.locationJa : method.locationKo)?.trim();
  return (!localizedLabel && Boolean(method.labelEn?.trim()))
    || (!localizedLocation && Boolean(method.locationEn?.trim()));
}

function localizedDescription(value: DescriptionReference, locale: PalworldLocale): { text: string; englishFallback: boolean } {
  const localized = (locale === "ja" ? value.descriptionJa : value.descriptionKo)?.trim();
  if (localized) return { text: localized, englishFallback: false };
  const english = value.descriptionEn?.trim();
  return { text: english ?? "", englishFallback: Boolean(english) };
}

function EnglishFallbackBadge({ locale }: { locale: PalworldLocale }) {
...
function SkillDescription({ locale, skill }: { locale: PalworldLocale; skill: PalworldSkill }) {
  const text = palworldI18n[locale];
  const description = localizedDescription(skill, locale);
  const usesEnglishFallback = usesEnglishNameFallback(skill, locale)
  ...
  return <div className="palworld-skill-detail-copy">
    <div className="palworld-badge-row">
      {usesEnglishFallback ? <EnglishFallbackBadge locale={locale} /> : null}
    ...
    <p>{description.text || text.originalDataUnavailable}</p>
  </div>;
}
...
function SpecialParentPair({
  ...
  return <li className="palworld-special-parent-pair">
    <span><code>{pair.parentAId}</code> + <code>{pair.parentBId}</code></span>
...
export function PalDetailModal({
  ...
  const description = detail ? localizedDescription(detail, locale) : { text: "", englishFallback: false };
        ...
        {detail ? (
          <article className="palworld-detail">
            <div className="palworld-detail-hero">
              ...
              <div><span className="palworld-detail-number">{formatPalNumber(detail.number)}</span><h3>{displayName}</h3><p>{multilingualNames(detail)}</p><div className="palworld-badge-row">{detail.elements.map((element) => <PalworldElementBadge element={element} locale={locale} size="md" key={element} />)}<Badge tone="warning">★ {detail.rarity}</Badge><Badge>{detail.variantType === "normal" ? text.normal : detail.variantType === "special" ? text.special : text.variantPal}</Badge></div></div>
            </div>
            <section><h4 data-ko={palworldI18n.ko.descriptionLabel} data-ja={palworldI18n.ja.descriptionLabel}>{text.descriptionLabel}</h4>{description.englishFallback ? <div className="palworld-badge-row"><EnglishFallbackBadge locale={locale} /></div> : null}<p>{description.text || text.originalDataUnavailable}</p></section>
            ...
            <section><h4 data-ko={palworldI18n.ko.drops} data-ja={palworldI18n.ja.drops}>{text.drops}</h4>{dropDetails?.length ? <div className="palworld-drop-list">{dropDetails.map((drop) => <div className="palworld-drop-row" key={drop.item.id}><button type="button" onClick={() => onOpenItem(drop.item.id)}>{referenceName(drop.item, locale)}</button><span>{text.dropQuantity} {drop.minQuantity === drop.maxQuantity ? drop.minQuantity : `${drop.minQuantity}–${drop.maxQuantity}`}{drop.dropRatePercent !== undefined ? ` · ${text.dropRate} ${drop.dropRatePercent}%` : ""}</span></div>)}</div> : detail.drops.length ? <div className="palworld-link-list">{detail.drops.map((drop) => <button type="button" onClick={() => onOpenItem(drop.id)} key={drop.id}>{referenceName(drop, locale)}</button>)}</div> : <p>{text.originalDataUnavailable}</p>}</section>
...
export function ItemDetailModal({
  ...
  const displayName = detail ? referenceName(detail, locale) : text.details;
  const description = detail ? localizedDescription(detail, locale) : { text: "", englishFallback: false };
  const nameEnglishFallback = detail ? usesEnglishNameFallback(detail, locale) : false;
  return (
        ...
        {detail ? (
          <article className="palworld-detail">
            <div className="palworld-detail-hero">
              ...
              <div><h3>{displayName}</h3><p>{multilingualNames(detail)}</p><div className="palworld-badge-row"><Badge tone="info">{categoryLabel(detail.category, locale)}</Badge><Badge tone="warning">★ {detail.rarity}</Badge>{nameEnglishFallback ? <EnglishFallbackBadge locale={locale} /> : null}</div></div>
            </div>
            <dl className="palworld-detail-list">
              ...
              <DataRow locale={locale} labelKo={palworldI18n.ko.descriptionLabel} labelJa={palworldI18n.ja.descriptionLabel}><span>{description.text || text.originalDataUnavailable}</span>{description.englishFallback ? <EnglishFallbackBadge locale={locale} /> : null}</DataRow>
            ...
            <section><h4 data-ko={palworldI18n.ko.craftingFacility} data-ja={palworldI18n.ja.craftingFacility}>{text.craftingFacility}</h4>{detail.craftingFacility ? <p>{referenceName(detail.craftingFacility, locale)} {usesEnglishNameFallback(detail.craftingFacility, locale) ? <EnglishFallbackBadge locale={locale} /> : null}</p> : <p>{text.originalDataUnavailable}</p>}</section>
            ...
            <section><h4 data-ko={palworldI18n.ko.acquisitionMethods} data-ja={palworldI18n.ja.acquisitionMethods}>{text.acquisitionMethods}</h4>{detail.acquisitionMethods.length ? <ul>{detail.acquisitionMethods.map((method, index) => {
              ...
              return <li key={`${method.type}-${index}`}><strong>{acquisitionLabel(method.type, locale)}</strong> · {acquisitionText(method, locale)}{location ? ` · ${location}` : ""}{usesEnglishAcquisitionFallback(method, locale) ? <> · <EnglishFallbackBadge locale={locale} /></> : null}</li>;
            })}</ul> : <p>{text.none}</p>}</section>
            <section><h4 data-ko={palworldI18n.ko.relatedItems} data-ja={palworldI18n.ja.relatedItems}>{text.relatedItems}</h4>{detail.relatedItems.length ? <div className="palworld-link-list">{detail.relatedItems.map((item) => <button type="button" onClick={() => onOpenItem(item.id)} key={item.id}>{referenceName(item, locale)}</button>)}</div> : <p>{text.originalDataUnavailable}</p>}</section>
            <section className="palworld-source"><h4 data-ko={palworldI18n.ko.source} data-ja={palworldI18n.ja.source}>{text.source}</h4><p>{detail.metadata.sourceName} · {detail.metadata.sourceRevision}</p><p>{text.gameVersion}: {detail.metadata.gameVersion} · {text.license}: {detail.metadata.license}</p></section>
          </article>
        ) : null}
      </ModalContent>
