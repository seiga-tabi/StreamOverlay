        ...
        {detail ? (
          <article className="palworld-detail">
            ...
            <section><h4 data-ko={palworldI18n.ko.dropPals} data-ja={palworldI18n.ja.dropPals}>{text.dropPals}</h4>{detail.dropPals.length ? <div className="palworld-link-list">{detail.dropPals.map((pal) => {
              ...
              return <button className="palworld-related-pal-link" type="button" onClick={() => onOpenPal(pal.id)} key={pal.id}><span className="palworld-related-pal-media"><PalworldMedia kind="pal" imageUrl={pal.imageUrl} alt={palDisplayName} locale={locale} /></span><span>{palDisplayName}</span></button>;
