export const publicI18n = {
  ko: {
    ...
    privacyCookiesBody: "로그인과 보안에 필요한 HttpOnly·SameSite 세션 쿠키를 사용하며 production에서는 Secure 속성을 적용합니다. 최근 검색, 즐겨찾기, 언어와 테마는 브라우저 localStorage에 저장됩니다. 브라우저 설정에서 쿠키·저장정보를 삭제할 수 있으나 로그인과 개인화 기능이 제한될 수 있습니다. 현재 제3자 행동기반 광고 쿠키는 사용하지 않습니다.",
  ...
  ja: {
    ...
    privacyCookiesBody: "ログインとセキュリティに必要なHttpOnly・SameSiteセッションCookieを使用し、productionではSecure属性を適用します。最近の検索、お気に入り、言語、テーマはブラウザのlocalStorageに保存します。ブラウザ設定から削除できますが、ログインや個人設定機能が制限される場合があります。現在、第三者による行動ターゲティング広告Cookieは使用していません。",
