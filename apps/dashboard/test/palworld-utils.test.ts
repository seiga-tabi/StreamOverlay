...
import {
  ...
  palworldUrl,
} from "../src/features/public-palworld/utils/routes";
import {
  acquisitionLabel,
  categoryLabel,
  elementLabel,
...
test("번역 snapshot 상태에 따라 현지어·영어 fallback·원문 없음 상태를 구분한다", () => {
  ...
  assert.deepEqual(resolvePalworldDescription({
    ...
    translation: {
      description: {
        ko: "machine_assisted",
        ja: "machine_assisted",
        sourceIntegrity: { ko: "source_anomaly", ja: "source_anomaly" },
      },
    },
  }, "ko"), {
    text: "에서 제작할 수 있다. [원문 누락]",
    status: "machine_assisted",
    sourceIntegrity: "source_anomaly",
  });
});
...
test("Pal 상세 농축 단계와 출현 시간 query는 허용값만 복원하고 canonicalize한다", () => {
  for (const stars of [0, 1, 2, 3, 4] as const) {
    assert.equal(
      palworldCondensationStarsFromParams(new URLSearchParams({ stars: String(stars) })),
      stars,
    );
  }
  for (const invalid of [
    ...
    assert.equal(palworldCondensationStarsFromParams(new URLSearchParams(invalid)), 0, invalid);
  }

  for (const period of ["all", "day", "night"] as const) {
    assert.equal(
      palworldSpawnPeriodFromParams(new URLSearchParams({ spawnPeriod: period })),
      period,
    );
  }
  for (const invalid of [
    "spawnPeriod=DAY",
    "spawnPeriod=daytime",
    "spawnPeriod=%20day%20",
    "spawnPeriod=night&spawnPeriod=night",
  ]) {
    assert.equal(palworldSpawnPeriodFromParams(new URLSearchParams(invalid)), "all", invalid);
  }

  const restored = palworldDetailSelectionFromParams(
    new URLSearchParams("q=아누비스&pal=anubis&stars=4&spawnPeriod=night"),
  );
  assert.deepEqual(restored.selection, { type: "pal", id: "anubis" });
  assert.equal(
    restored.canonicalParams.toString(),
    "q=%EC%95%84%EB%88%84%EB%B9%84%EC%8A%A4&pal=anubis&stars=4&spawnPeriod=night",
  );
  assert.equal(restored.changed, false);

  const defaults = palworldDetailSelectionFromParams(
    new URLSearchParams("pal=anubis&stars=0&spawnPeriod=all"),
  );
  assert.equal(defaults.canonicalParams.toString(), "pal=anubis");
  assert.equal(defaults.changed, true);

  const invalid = palworldDetailSelectionFromParams(
    new URLSearchParams("pal=anubis&stars=5&spawnPeriod=dawn"),
  );
  assert.equal(invalid.canonicalParams.toString(), "pal=anubis");
  assert.equal(invalid.changed, true);

  const nonPal = palworldDetailSelectionFromParams(
    new URLSearchParams("item=pal-sphere&stars=4&spawnPeriod=night"),
  );
  assert.deepEqual(nonPal.selection, { type: "item", id: "pal-sphere" });
  assert.equal(nonPal.canonicalParams.toString(), "item=pal-sphere");
  assert.equal(nonPal.changed, true);
});

test("농축 능력치는 계산하지 않고 서버가 제공한 단계 값만 기본 능력치에 병합한다", () => {
  ...
  assert.deepEqual(applyPalworldCondensationStage(baseStats), baseStats);
  assert.notEqual(applyPalworldCondensationStage(baseStats), baseStats);

  const applied = applyPalworldCondensationStage(baseStats, {
    stars: 4,
    characterRank: 5,
    partnerSkillRank: 5,
    stats: [
      { stat: "hp", baseValue: 100, value: 117 },
      { stat: "shotAttack", baseValue: 90, value: 123 },
    ],
    workSuitabilities: [],
  });
  ...
  assert.equal(applied.attack, 80);
  assert.equal(applied.defense, 70);
  assert.equal(applied.stamina, 120);
});
