...
test("Pal 도감 상세 필터는 서버 facet만 아이콘·텍스트 단일 선택으로 렌더한다", () => {
  ...
  assert.match(html, /palworld-element-icon/u);
...
test("Pal 도감 적용 chip과 결과 toolbar는 제거 label·0/0 페이지·정렬을 분리해 표시한다", () => {
  ...
  assert.match(toolbar, /조건에 맞는 Pal 0종 · 0\/0 페이지/u);
...
test("월드 지도 일반 스폰 layer는 cluster 좌표를 확대 배율에 맞춰 표시한다", () => {
  ...
  assert.match(markup, /r="0\.0045"/u);
});
