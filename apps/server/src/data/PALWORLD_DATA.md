...
- 현재 release는 외부 이미지 URL이나 운영자 source 원본을 포함하지 않는다. 실제 이미지가 반입되기 전에는 프로젝트의 일반 fallback 그래픽을 사용해야 한다.
