...
} = await import("../dist/data/palworld-pak-promotion.js");
...
test("결정적 snapshot artifact를 디스크 staging에서 재검증해 shadow API service를 구성한다", async () => {
  ...
  try {
    ...
    const importReport = {
      ...
      blockers: ["OFFICIAL_EN_LOCALE_NOT_PROVIDED"],
      coverageLimitations: [],
      counts: {},
      sourceCounts: {},
      localeCoverage: {},
    ...
    const files = new Map([
      ...Object.entries(fixture.input.artifacts)
        .filter(([file]) => file !== "locales/en.json")
    ...
    const dataRoot = await mkdtemp(
      path.join(await realpath(tmpdir()), "palworld-pak-promoted-")
    );
    try {
      const promoted = await promotePalworldPakRuntime({
        stagingRoot: root,
        dataRoot
      });
      assert.equal(promoted.active.format, "operator_pak_v1");
      assert.equal(promoted.active.release, "2.0.0");
      assert.equal(
        (await loadPalworldActiveRuntime({ dataRoot })).manifest
          .releaseDirectory,
        promoted.active.releaseDirectory
      );
    } finally {
      await rm(dataRoot, { recursive: true, force: true });
    }

    const invalidReportBytes = Buffer.from(JSON.stringify({
      ...importReport,
      blockers: ["ITEM_ICON_EXPORT_INCOMPLETE"]
    }));
    const invalidManifest = structuredClone(manifest);
    const reportArtifact = invalidManifest.artifacts.find(
      (artifact) => artifact.kind === "import-report"
    );
    reportArtifact.sha256 = sha256(invalidReportBytes);
    reportArtifact.bytes = invalidReportBytes.length;
    await writeFile(
      path.join(root, "import-report.json"),
      invalidReportBytes
    );
    await writeFile(
      path.join(root, PALWORLD_PAK_RUNTIME_MANIFEST_FILE),
      deterministicPalworldPakRuntimeManifestJson(invalidManifest)
    );
    await assert.rejects(
      validatePalworldPakCandidateStagingRoot({ stagingRoot: root }),
      /공식 EN locale 누락만/u
    );
  } finally {
...
test("stale locale·placeholder를 차단하고 공식 EN 누락을 명시적으로 격리한다", () => {
  ...
  const removeOfficialEn = (value) => {
    if (value === null || typeof value !== "object") return;
    if (Object.hasOwn(value, "enStatus")) {
      value.en = null;
      value.enStatus = "missing_source";
      delete value.enRichTextStatus;
      delete value.enRichText;
    }
    for (const child of Object.values(value)) removeOfficialEn(child);
  };
  for (const file of ["paldex.json", "items.json", "skills.json"]) {
    replaceArtifact(missingEn, file, removeOfficialEn);
  }
  const withoutOfficialEn = adaptPalworldPakCandidateToSnapshot(missingEn.input);
  assert.equal(
    withoutOfficialEn.snapshot.pals.some((pal) => pal.nameEn !== undefined),
    false
  );
  assert.equal(
    withoutOfficialEn.snapshot.items.some((item) => item.nameEn !== undefined),
    false
  );
  assert.equal(
    withoutOfficialEn.snapshot.skills.some((skill) => skill.nameEn !== undefined),
    false
  );
});

test("미해결 item rich text는 해당 설명만 격리하고 item record를 유지한다", () => {
  const fixture = canonicalFixture();
  replaceArtifact(fixture, "items.json", (value) => {
    for (const locale of ["ko", "ja", "en"]) {
      value.records[0].description[`${locale}RichText`].unresolved = [{
        code: "UNKNOWN_TAG",
        offset: 0,
        token: "<unknown>"
      }];
  ...
  const result = adaptPalworldPakCandidateToSnapshot(fixture.input);
  const item = result.snapshot.items[0];
  assert.equal(item.descriptionKo, undefined);
  assert.equal(item.descriptionJa, undefined);
  assert.equal(item.descriptionEn, undefined);
  assert.deepEqual(item.translation.description, {
    ko: "missing_source",
    ja: "missing_source"
  });
});
