...
test("rollback 검증은 손상된 retained 이미지보다 공개 참조 제거를 우선한다", async () => {
  ...
  try {
    ...
    assert.equal(rollbackRelease.artifact.records.every((pal) => pal.imageUrl === undefined), true);
...
test("data integrity와 image asset gate는 서로 독립된 coverage를 보고한다", async () => {
  ...
  assert.equal(report.images.status, "blocked_by_license");
  assert.equal(report.images.policyStatus, "operator_acknowledged");
  assert.equal(report.images.sourceMappings, 0);
  assert.equal(report.images.mappedPals, 287);
  assert.equal(report.images.readyImages, 0);
  assert.equal(report.images.uniqueImageFiles, 0);
  assert.equal(report.images.fallbackPals, 287);
...
test("이미지 manifest는 운영자 source reference와 287개 fallback을 명시한다", async () => {
  const artifact = assertPalworldPaldexArtifact(await json("paldex.json"));
  ...
  assert.equal(images.entries.every((entry) => entry.sourceReference === `operator-export-${entry.sourceInternalId}`), true);
  const sourceMap = await json("image-source-map.json", mappingRoot);
  assert.equal(sourceMap.entries.length, 0);
});
...
test("operator policy와 source lock 권리 상태를 분리하고 policy 없는 공개 전환을 차단한다", async () => {
  ...
  unverifiedReadyGate.imageAssetGate.status = "operator_acknowledged";
  unverifiedReadyGate.imageAssetGate.publicActivationAllowed = true;
  unverifiedReadyGate.imageAssetGate.technicalPassed = true;
  unverifiedReadyGate.imageAssetGate.passed = true;
  unverifiedReadyGate.imageAssetGate.readyImages = 287;
  unverifiedReadyGate.imageAssetGate.fallbackPals = 0;
  assert.throws(
    () => assertPalworldImageRightsGate({ sourceLock, imagesManifest, manifest: unverifiedReadyGate, policy }),
    /상태/
  );
});
