...
const {
  auditPalworldSpawnDistributionRows,
...
test("일반 스폰 mapping은 고정 source와 검수된 correction만 허용한다", async () => {
  ...
  assert.equal(
    assertPalworldSpawnMapping(mapping).expectedDistributionAudit.sourceRows,
    365
  );
  assert.throws(
...
test("Paldex 분포는 strict spawner에 병합하지 않고 exact coverage 통계만 검증한다", async () => {
  const paldex = JSON.parse(await readFile(releasePaldexPath, "utf8"));
  const audit = auditPalworldSpawnDistributionRows({
    paldex,
    rows: {
      BluePlatypus: {
        dayTimeLocations: {
        ...
        nightTimeLocations: {
          Locations: [{ X: 4, Y: 5, Z: 6 }, { X: 7, Y: 8, Z: 9 }],
          Radius: 15_000
        }
      }
    },
    spawnSourceInternalIds: ["BluePlatypus"]
  });
  assert.deepEqual(audit, {
    ...
    rowsWithNighttime: 1,
    daytimeLocations: 1,
    nighttimeLocations: 2,
    exactPaldexRows: 1,
    paldexRowsWithoutDistribution: paldex.records.length - 1,
    distributionRowsWithoutPaldex: 0,
    spawnPalsWithDistribution: 1,
    spawnPalsWithoutDistribution: 0
  });
  assert.throws(
    () => assertPalworldSpawnDistributionAudit(
      audit,
      { ...audit, daytimeLocations: audit.daytimeLocations + 1 }
    ),
    /expectedDistributionAudit\.daytimeLocations/u
  );
  assert.throws(
    () => auditPalworldSpawnDistributionRows({
      paldex,
      rows: {
        BluePlatypus: {
          dayTimeLocations: { Locations: [], Radius: 0 },
          nightTimeLocations: { Locations: [], Radius: 0 },
          bossLocations: []
        }
      },
      spawnSourceInternalIds: ["BluePlatypus"]
