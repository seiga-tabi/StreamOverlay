...
const HOME_I18N_PATH = fileURLToPath(new URL(
  "../../dashboard/src/features/public-home/i18n/home-i18n.ts",
  import.meta.url
));
const homeI18nSource = readFileSync(HOME_I18N_PATH, "utf8");

/** home-i18n.ts 의 로케일 블록에서 seoTitle·seoDescription 을 그대로 꺼냅니다. */
function homeSeoCopy(locale) {
  const block = new RegExp(`\\n  ${locale}: \\{([\\s\\S]*?)\\n  \\}`, "u").exec(homeI18nSource);
  assert.ok(block, `${locale} 블록을 찾지 못했습니다 — home-i18n.ts 구조가 바뀌었습니다`);
  const read = (key) => {
    const found = new RegExp(`\\n    ${key}: "((?:[^"\\\\]|\\\\.)*)"`, "u").exec(block[1]);
    assert.ok(found, `${locale}.${key} 를 찾지 못했습니다`);
