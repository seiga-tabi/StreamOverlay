...
test("일반 스폰 artifact가 없으면 다른 Palworld API를 중단하지 않는다", async () => {
  ...
  assert.equal(spawn.res.headers["Cache-Control"], "no-store");
  ...
  const bosses = await request(handler(), "/api/palworld/map/markers?world=main");
  assert.equal(bosses.res.statusCode, 200);
  assert.equal(bosses.body.state, "data_unavailable");
});
...
test("active spawn provider는 Pal을 exact join하고 빈 위치 상태를 구분한다", async () => {
  ...
  assert.match(ready.res.headers["Cache-Control"], /max-age=/u);
  ...
  assert.throws(
    ...
    (error) => error instanceof PalworldSpawnArtifactError
      && error.code === "PALWORLD_SPAWN_ARTIFACT_NOT_ACTIVE"
  );
  assert.throws(
    () => createPalworldSpawnArtifact({
      ...artifact(),
