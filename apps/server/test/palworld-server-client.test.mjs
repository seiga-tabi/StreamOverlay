...
function createHttpsClient(overrides = {}) {
  ...
  const client = new PalworldServerClient({
    ...
    publicHttpsSelfService: overrides.publicHttpsSelfService,
...
test("공개 HTTPS 자가 등록은 비 allowlist hostname의 기본 443 origin만 허용한다", async () => {
  const { client, calls } = createHttpsClient({
    allowedOrigins: [],
    publicHttpsSelfService: true
  });

  assert.deepEqual(client.evaluateBaseUrlPolicy("HTTPS://PAL.EXAMPLE:443/"), {
    ok: true,
    baseUrl: "https://pal.example",
    hostname: "pal.example",
    protocol: "https:",
  ...
  assert.deepEqual(calls.map((call) => call.url.href), [
    "https://pal.example/v1/api/info",
    "https://pal.example/v1/api/metrics"
  ]);
});
...
test("공개 HTTPS 자가 등록은 HTTP·custom port·IP literal·비정상 hostname을 거부한다", () => {
  ...
  const deniedUrls = [
    ...
    "https://localhost",
    "https://pal_server.example",
    "https://pal.example."
  ];
  for (const url of deniedUrls) {
    assert.throws(
      () => client.normalizeBaseUrl(url),
      (error) => assertClientError(error, "origin_not_allowed", "url_policy"),
      url
    );
  }
  assert.deepEqual(client.evaluateBaseUrlPolicy("https://pal.example:8212"), {
    ok: false,
    errorCode: "origin_not_allowed",
    reason: "self_service_default_port_required"
  });
});

test("URL 정책은 backslash와 percent-encoding 정규화 우회를 거부한다", () => {
  const { client } = createHttpsClient({
    allowedOrigins: [],
    publicHttpsSelfService: true
  });
  for (const url of [
    String.raw`https:\\pal.example`,
    "https://pal%2eexample",
    "https://pal.example/%2e"
  ]) {
    assert.throws(
      () => client.normalizeBaseUrl(url),
      (error) => assertClientError(error, "invalid_url", "url_policy"),
      url
    );
  }
});
...
test("공개 HTTPS 자가 등록은 모든 DNS 결과가 public일 때만 허용한다", async () => {
  let requestCount = 0;
  const { client } = createHttpsClient({
    allowedOrigins: [],
  ...
  await assert.rejects(
    () => client.probe({ baseUrl: "https://pal.example", adminPassword: "secret-password" }),
    (error) => {
      assertClientError(error, "address_blocked", "dns_tcp");
      assert.equal(error.policyReason, "self_service_non_public_address");
      return true;
    }
  );
  assert.equal(requestCount, 0, "운영자 CIDR가 있어도 self-service origin에는 적용하면 안 됩니다.");
});

test("공개 HTTPS 자가 등록은 public IPv4-mapped IPv6 주소도 우회로 사용하지 못한다", async () => {
  let requestCount = 0;
  const { client } = createHttpsClient({
    allowedOrigins: [],
    publicHttpsSelfService: true,
    resolveHostname: async () => [{ address: "::ffff:8.8.8.8", family: 6 }],
    requestPinned: async () => {
      requestCount += 1;
      return jsonResponse(VALID_INFO);
    }
  ...
  await assert.rejects(
    ...
    (error) => {
      assertClientError(error, "address_blocked", "dns_tcp");
      assert.equal(error.policyReason, "self_service_mapped_ipv4_address");
      return true;
    }
  );
  assert.equal(requestCount, 0);
...
test("공개 HTTPS 자가 등록은 probe마다 DNS를 재검증해 rebinding을 차단한다", async () => {
  let resolveCount = 0;
  let requestCount = 0;
  const { client } = createHttpsClient({
    allowedOrigins: [],
    ...
    requestPinned: async (request) => {
      requestCount += 1;
      return request.url.pathname.endsWith("/info")
        ? jsonResponse(VALID_INFO)
        : jsonResponse(VALID_METRICS);
    }
  });

  const first = await client.probe({ baseUrl: "https://pal.example", adminPassword: "secret-password" });
  ...
  assert.equal(requestCount, 2, "두 번째 probe에서는 Authorization 요청 전에 차단해야 합니다.");
});

test("운영자 allowlist만 private CIDR와 public HTTPS custom port 예외를 부여한다", async () => {
  const privateAddress = [{ address: "10.20.30.40", family: 4 }];
  const { client: unlistedPrivate } = createHttpsClient({
    allowedOrigins: [],
    allowedCidrs: ["10.20.0.0/16"],
    publicHttpsSelfService: true,
    resolveHostname: async () => privateAddress
  ...
  await assert.rejects(
    () => unlistedPrivate.probe({ baseUrl: "https://pal.example", adminPassword: "secret-password" }),
    (error) => assertClientError(error, "address_blocked", "dns_tcp")
  );

  const { client: customPort, calls } = createHttpsClient({
    allowedOrigins: ["https://pal.example:8212"],
    publicHttpsSelfService: true
  });
  assert.equal(
    customPort.evaluateBaseUrlPolicy("https://pal.example:8212").basis,
    "operator_allowlist"
  );
  const result = await customPort.probe({
    baseUrl: "https://pal.example:8212",
    adminPassword: "secret-password"
  });
  assert.equal(result.state, "online");
  assert.equal(calls[0].url.port, "8212");
});
...
test("PalworldServerClient는 DNS와 두 endpoint가 하나의 합산 deadline을 공유한다", async () => {
  ...
  const client = new PalworldServerClient({
    ...
    resolveHostname: () => delayed([{ address: "8.8.8.8", family: 4 }], 20),
    requestPinned: (request) => {
      requestTimeouts.push(request.timeoutMs);
      return request.url.pathname === "/v1/api/info"
        ? delayed(jsonResponse(VALID_INFO), 30)
        : delayed(jsonResponse(VALID_METRICS), 1_000);
    }
  });
  ...
  try {
    ...
    assert.deepEqual(result.info, { serverName: "세이가 Palworld", version: "v1.0.0" });
    assert.equal(result.metrics, undefined);
    assert.equal(elapsedMs >= 400, true);
    assert.equal(elapsedMs < 900, true);
    assert.equal(requestTimeouts.length, 2);
    assert.equal(requestTimeouts[0] < 500, true, "DNS 시간만큼 /info 제한이 줄어야 합니다.");
