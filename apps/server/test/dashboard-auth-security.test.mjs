...
test("관리자는 승인된 스트리머의 대시보드 사용 권한을 별도로 변경할 수 있다", async () => {
  await withAuthConfig(async () => {
    ...
    const store = {
      ...
      setStreamerRiotIdDashboardEnabled(input) {
        ...
        if (input.dashboardEnabled) {
          request.dashboardSlug ||= "seiga";
          request.dashboardKey ||= "sdk_dashboard_test";
        }
    ...
    assert.equal(allowedAuthBody.streamer.dashboardSlug, "seiga");
    assert.equal(allowedAuthBody.streamer.dashboardKey, "sdk_dashboard_test");
    assert.equal(allowedAuthBody.streamer.dashboardPath, "/dashboard/seiga/sdk_dashboard_test");
    assert.notEqual(allowedAuthBody.streamer.dashboardKey, allowedAuthBody.streamer.overlayKey);
    const streamerCookie = cookieHeader(allowedAuthRes.headers["Set-Cookie"]);
    assert.equal(streamerCookie.startsWith(`${STREAMER_DASHBOARD_SESSION_COOKIE}=`), true);

    const matchingTenantReq = createRequest("GET", "/api/dashboard/auth/status?surface=streamer", undefined, {
      origin: DASHBOARD_ORIGIN,
      cookie: streamerCookie,
      "x-streamops-dashboard-surface": "streamer",
      "x-streamops-streamer-slug": "seiga",
      "x-streamops-dashboard-key": "sdk_dashboard_test"
    });
    const matchingTenantRes = createResponse();
    await handler(matchingTenantReq, matchingTenantRes);
    assert.equal(matchingTenantRes.statusCode, 200);

    const wrongTenantReq = createRequest("GET", "/api/dashboard/auth/status?surface=streamer", undefined, {
      origin: DASHBOARD_ORIGIN,
      cookie: streamerCookie,
      "x-streamops-dashboard-surface": "streamer",
      "x-streamops-streamer-slug": "another-streamer",
      "x-streamops-dashboard-key": "sdk_wrong"
    });
    const wrongTenantRes = createResponse();
    await handler(wrongTenantReq, wrongTenantRes);
    assert.equal(wrongTenantRes.statusCode, 403);
    assert.equal(JSON.parse(wrongTenantRes.body).code, "STREAMER_TENANT_MISMATCH");

    const disableReq = createRequest("POST", "/api/participation/streamer-riot-id-requests/dashboard-access", {
      requestId: "riotreq-1",
      dashboardEnabled: false
    }, {
      origin: DASHBOARD_ORIGIN,
    ...
    const revokedSessionRes = createResponse();
    await handler(revokedSessionReq, revokedSessionRes);
    assert.equal(revokedSessionRes.statusCode, 401);
  });
});
      ...
      store: {
        listApprovedStreamerRiotIds() {
          return [{
            id: "request-1",
            twitchUserId: "1234",
            twitchLogin: "hideonbush",
            twitchDisplayName: "Hide on bush",
            riotGameName: "HideOnBush",
            riotTagLine: "KR1",
            normalizedRiotId: "hideonbush#kr1",
            status: "approved",
...
test("공개 LoL 전적 캐시는 Twitch 방송 상태를 매 요청마다 갱신한다", async () => {
  await withAuthConfig(async () => {
    ...
    const firstReq = createRequest("GET", "/api/lol/profile?riotId=Seiga%23sei", undefined, { origin: DASHBOARD_ORIGIN });
    const firstRes = createResponse();
    await handler(firstReq, firstRes);
...
test("잘못 인코딩된 쿠키는 인증 상태 조회를 실패시키지 않는다", async () => {
  await withAuthConfig(async () => {
    let publicSessionId = "not-called";
    const handler = createHttpHandler({
      store: {},
      twitchAuth: {},
      publicTwitchAuth: {
...
test("스트리머 dashboard 세션은 허용된 운영 API만 사용할 수 있다", async () => {
  await withAuthConfig(async () => {
    ...
    for (const pathname of [
      ...
      const singletonWriteReq = createRequest("POST", pathname, {}, {
        cookie,
        origin: DASHBOARD_ORIGIN,
        "x-streamops-dashboard-surface": "streamer",
        "x-streamops-csrf": session.csrfToken
      });
      const singletonWrite = authorizeHttpRequest(singletonWriteReq, pathname, sessions);
