...
test("펠월드 공개 API는 인증 없이 meta와 cache header를 제공한다", async () => {
  ...
  assert.equal(res.headers["X-Palworld-Data-Version"], "sample-baseline");
  assert.equal(body.counts.pals, 12);
});
...
test("Pal과 아이템 목록 API는 filter와 pagination을 적용한다", async () => {
  ...
  assert.deepEqual(pals.body.items.map((pal) => pal.id), ["anubis"]);
